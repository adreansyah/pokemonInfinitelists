...
const Index = () => {
    ...
    const { loading, error, data: { pokemons = [] } = {}, fetchMore } = useQuery(POKEMONS_QUERY, {
        variables: { first: 16 },
    })
    ...
    useEffect(() => {
        keywords.length || pokemons.length % 16 !== 0 ? setHasMore(false) : setHasMore(true)
    }, [keywords, pokemons.length])

    return (
        <Fragment>
            <Header onChange={({ target: { value } }) => setKeywords(value)} onDelete={() => setKeywords("")} keywords={keywords} />
            <Box my={2}>
                <Container>
                    <InfiniteScroll
                        refs="pokemonList"
